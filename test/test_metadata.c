...
START_TEST(test_metadata_disk_provider_write_failure_serves_in_memory) {
	...
	oidc_test_http_response_t resp = {.status_code = 200, .content_type = "application/json", .body = NULL};
	oidc_test_http_server_t *srv = oidc_test_http_server_start(r->pool, &resp);
	ck_assert_ptr_nonnull(srv);

	/* the issuer is the local fixture, so the .well-known retrieval lands there; the response
	 * body can only be filled in now that the kernel has assigned the port (the fixture keeps
	 * a reference to the response struct, not a copy) */
	...
	/* occupy the provider file's path with a directory: the temp-file write inside the
	 * metadata directory succeeds but the rename over a directory cannot, root or not */
	const char *provider_path = oidc_metadata_provider_file_path(r, issuer);
	ck_assert_int_eq(apr_dir_make(provider_path, APR_OS_DEFAULT, r->pool), APR_SUCCESS);

	oidc_json_t *j = NULL;
	ck_assert_msg(oidc_metadata_provider_get(r, c, issuer, &j, TRUE) == TRUE,
		      "a failed metadata cache write must not fail a completed, validated retrieval");
	ck_assert_ptr_nonnull(j);
	...
	const oidc_test_http_captured_t *cap = oidc_test_http_server_wait(srv);
	ck_assert_str_eq(cap->method, "GET");
	ck_assert_ptr_nonnull(_oidc_strstr(cap->path, "/.well-known/openid-configuration"));
...
int main(void) {
	...
	tcase_add_test(disk, test_metadata_disk_provider_write_failure_serves_in_memory);
